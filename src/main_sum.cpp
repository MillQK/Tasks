...
#pragma omp parallel for reduction(+:sum)
...

    {
        ...
        context.activate();
        {
            ...
            gpu::gpu_mem_32u input_vram, output;
            ...
            for (int i = 0; i < benchmarkingIters; ++i) {
                ...
                output.writeN(&result, 1);

                kernel.exec(gpu::WorkSize(workGroupSize, global_work_size),
                            input_vram, output, ocl::LocalMem(workGroupSize * sizeof(unsigned int)), n);

                output.readN(&result, 1, 0);
                EXPECT_THE_SAME(reference_sum, result, "GPU OpenCL result should be consistent!");
                t.nextLap();
            }
